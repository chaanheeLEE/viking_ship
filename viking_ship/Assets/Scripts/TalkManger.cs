...
public class TalkManger : MonoBehaviour
{
    ...
    public TMPro.TextMeshProUGUI tailor_button_text;
    ...
    {
        // 배 만들기 퀘스트 활성화 되면
        if (quest3Active == true)
        {
            tailor_text.text = tailorQuestString[0];
            tailor_button_text.text = "교환";

            // 인벤토리의 풀의 개수가 5이상이면 밧줄 한 개로 교환 가능
            if (inventoryGrassManager.user.items[2].value >= 5)
            {
                // 풀 5개랑 밧줄 1개 교환
                inventoryRopeManager.user.items[3].value = inventoryRopeManager.user.items[3].value + inventoryGrassManager.user.items[2].value / 5;
                // 풀이 5의 배수가 아니면 풀의 남기 때문에 나머지 계산
                if (inventoryGrassManager.user.items[2].value / 5 != 0)
                    inventoryGrassManager.user.items[2].value = inventoryGrassManager.user.items[2].value % 5;
                // 풀이 5의 배수이면 0개됨
                else
                    inventoryGrassManager.user.items[2].value = 0;
            }
        }

        // 배 만들기 퀘스트 활성화 안되면 일반 대화
        else
        {
